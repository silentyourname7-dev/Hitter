...
def e(emoji):
    """Return plain emoji (custom IDs may not work in all chats).
    For maximum compatibility, use regular emojis which work everywhere.
    To enable custom emojis: Make sure bot has 'Use Emoji' permissions.
    """
    # Fallback to plain emoji - works in all chats
    return emoji
    
    # Uncomment below ONLY if using in chat with custom emoji support:
    # return PREMIUM_EMOJI_IDS.get(emoji, emoji)

def ec(emoji, count=1):
    return " ".join([emoji for _ in range(count)])
