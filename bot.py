"""
⚡ ASIF HITTER — PREMIUM TELEGRAM BOT v3.1 ⚡
Production-Ready | Professional Output | Full Proxy & Key System
Dev: Asif Sakhani (@Asifsakhani786)

Fixes:
- Proxy checking one-by-one with proper timing
- /redeem command for key redemption
- ASIF- prefix keys
- Anti double-redeem protection
- Professional /hit output format
- Card injection without skipping
...
async def load_settings() -> Dict[str, Any]:
    async with SETTINGS_LOCK:
        ...
        defaults = {"log_channel": "@your_channel", "max_proxies": 6, "version": "3.1"}
...
async def user_has_active_plan(uid: int) -> bool:
    return await is_premium(uid)
...
async def check_single_proxy(proxy: str) -> Tuple[bool, str]:
    """Check ONE proxy with proper timing"""
...
class StripeChecker:
    ...
    def _proxy_url(self):
        if self.proxy_type == "http" and not self.socks_type: return self.proxy
        return None
...
{e('🤖')} <b>Status:</b> {badge}
{e('💎')} <b>Version:</b> v{await get_setting('version', '3.1')}
...
async def cmd_addproxy(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for pline in lines:
        ...
        await asyncio.sleep(1.5)  # Proper delay between checks
...
async def cmd_proxy(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if len(txt) > 4000:
        ...
        for i, part in enumerate(parts):
            ...
            if i == 0:
                await st.edit_text(header + part, parse_mode=ParseMode.HTML)
            else:
...
async def cmd_hit(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Results lines for final output
    result_lines = []
...
{e('📊')} {e('🟢')} Charged: {len(charged_cards)} | {e('🔵')} Live: {len(live_cards)} | 🟡 3DS: {three_ds} | 🔴 Declined: {declined}
"""
        try:
        ...
        # Build result line
        cc_masked = f"{card['cc'][:6]}xxxxxx{card['cc'][-4:]}"
        
        if sts == "CHARGED":
            charged_cards.append(r["card"])
            emoji_status = f"{e('🟢')} Charged ✅"
            result_lines.append(f"<b>CC:</b> <code>{r['card']}</code>\n<b>Status:</b> {emoji_status}\n<b>Response:</b> {h(r['msg'])}")
        elif sts == "LIVE":
            live_cards.append(r["card"])
            emoji_status = f"{e('🔵')} Live CVC Match"
            result_lines.append(f"<b>CC:</b> <code>{r['card']}</code>\n<b>Status:</b> {emoji_status}\n<b>Response:</b> {h(r['msg'])}")
            declined += 1  # Count as failed but save
        elif sts == "3DS":
            three_ds += 1
        elif sts == "HCAPTCHA":
            hcaptcha += 1
...
{e('🔵')} Live (CVC Match): <b>{len(live_cards)}</b>
🟡 3DS: <b>{three_ds}</b>
    ...
    if log_channel and (charged_cards or live_cards):
        try:
            await ctx.bot.send_message(
                ...
                f"{e('🟢')} Charged: {len(charged_cards)} | {e('🔵')} Live: {len(live_cards)}",
                parse_mode=ParseMode.HTML
            )
        except: pass
...
async def cmd_redeem(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Redeem premium key — /redeem ASIF-XXXX-XXXX"""
    uid = update.effective_user.id
    
    # Check if user already has active premium
    if await user_has_active_plan(uid):
    ...
    if not ctx.args:
        await update.message.reply_text(
            ...
            f"{e('🔑')} <b>Example:</b> <code>/redeem ASIF-1234-5678</code>",
            parse_mode=ParseMode.HTML
        )
        return
    ...
    # Validate key format — must start with ASIF
    ...
    if kdata.get("used"):
        await update.message.reply_text(
            ...
            f"{e('ℹ️')} Try another key or contact admin.",
            parse_mode=ParseMode.HTML
        )
        return
...
async def cmd_genkey(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # /genkey 10 24 1  = 10 keys, 24 hours, 1 use per key
    # /genkey 10 7 0   = 10 keys, 7 days (0 means days mode)
    ...
    if len(ctx.args) < 2:
        await update.message.reply_text(
            ...
            f"{e('ℹ️')} Keys format: <code>ASIF-XXXX-XXXX</code>",
            parse_mode=ParseMode.HTML
        )
        return
    ...
    try:
        ...
        mode = int(ctx.args[2]) if len(ctx.args) > 2 else 1  # 1=hours, 0=days
    except:
    ...
    for _ in range(count):
        k = f"ASIF-{random.randint(1000,9999)}-{random.randint(1000,9999)}"
        # Ensure unique
        while k in kd["keys"]:
            k = f"ASIF-{random.randint(1000,9999)}-{random.randint(1000,9999)}"
    ...
    txt += f"{e('🔢')} <b>Format:</b> <code>ASIF-XXXX-XXXX</code>\n"
...
def main():
    print("🚀 Asif Hitter Bot v3.1 starting...")
