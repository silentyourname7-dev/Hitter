"""
⚡ ASIF HITTER — PREMIUM TELEGRAM BOT v4.2 FINAL ⚡
...
# ═══════════════════════════ CONFIG ═══════════════════════════
BOT_TOKEN = os.environ.get("BOT_TOKEN", "8737062520:AAE46FJfbX-_l7wVUSEaGELB85cjnv1kR5M")
...
def _rc():
    CACHE["settings"] = _lj(SETTINGS_FILE,{"log_channel":"","max_proxies":6,"version":"4.2"})
...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        # FIXED: Removed 'e()' wrapper from InlineKeyboardButton text. Standard emojis ONLY.
        kb=[
        ...
        if adm: kb.append([InlineKeyboardButton("⚡ Admin Panel",callback_data="admin_panel")])
        
        txt=f"""{ec('🚀',3)} <b>ASIF HITTER</b> {ec('🚀',3)}

┌─────────────────────────────┐
│ {e('🤖')} Status: {badge}
│ {e('💎')} Version: <code>v{gs('version','4.2')}</code>
...
async def cmd_redeem(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if key not in kd["keys"]:
            await update.message.reply_text(f"{e('❌')} Invalid key!",parsemode=ParseMode.HTML)
            return
        kdata=kd["keys"][key]
        if kdata.get("used"):
...
def main():
    ...
    print("✅ ASIF HITTER v4.2 RUNNING!")
